...
func (bt *BinaryTree) insertNode(node, cur *Node) {
	if node.key < cur.key { // insert Left
		if cur.left == nil {
			cur.left = node
			return
		}
...
func (bt *BinaryTree) RemoveNode(node *Node) {
	...
	if parent != nil { // удаляем не корень
		...
		bt.root = minimum // удаляем корень
	}

	if minimum != node.right { // если минимум в правом поддереве не сам правый потомок
		minParent.left = minimum.right
		minimum.right = node.right
	}
	minimum.left = node.left // переназначаем левое поддерево удаляемого узла
}
...
func (bt *BinaryTree) searchMinimum(node *Node) (*Node, *Node) {
	minimum := node.right // минимум в правом поддереве
	minParent := node     // родитель минимума
